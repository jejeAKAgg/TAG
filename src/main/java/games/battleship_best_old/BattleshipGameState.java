...
public class BattleshipGameState extends AbstractGameState implements IPrintable, IGridGameState {
    ...
    // Heat-map storing the probability of the presence of a ship in the case x, y
    ...
    protected GameType _getGameType() {
        return GameType.BESTOLDBattleship;
    }
    ...
    /**
     * Create a heat-map based on CSP
    */
    public void updateHeatMap(int iterations, int playerID) {
        GridBoard knownShots = (playerID == 0) ? player0ShotGrid : player1ShotGrid;
        
        int size = knownShots.getWidth();
        this.heatMap = new double[size][size];
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            for (int x = 0; x < size; x++) {
                for (int y = 0; y < size; y++) {
                    if (((BoardNode)sample.getElement(x, y)).getComponentName().equals(BattleshipConstants.SHIP)) {
                        this.heatMap[x][y] += 1.0;
                    }
                }
            }
        }
    ...
    protected double _getHeuristicScore(int playerId) {
        BattleshipParameters params = (BattleshipParameters) getGameParameters();
        double score = getGameScore(playerId);
        double probabilityInference = 0;
        
        if (this.heatMap != null) {
            probabilityInference = calculateHeatMapInference(playerId);
        }

        return score + (getClusteringBonus(playerId) * params.heuristicWeight * 0.5)
                     + (probabilityInference * params.heuristicWeight);
    }
    ...
    private double calculateHeatMapInference(int playerId) {
        ...
        return hitCount > 0 ? totalInferenceValue / hitCount : 0;
    }
